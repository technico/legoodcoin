...
CREATE TABLE annonce (id INT AUTO_INCREMENT, etat_de_validation VARCHAR(8) DEFAULT 'wait', ville VARCHAR(30) NOT NULL, code_postal VARCHAR(5) NOT NULL, contenu TEXT NOT NULL, titre VARCHAR(50) NOT NULL, prix DECIMAL(65,2), est_abusif INT DEFAULT '0', type_annonce VARCHAR(7) NOT NULL, categorie INT NOT NULL, region INT NOT NULL, departement INT NOT NULL, telephone VARCHAR(10), annonceur INT, validee_par INT, date_control DATETIME, INDEX validee_par_idx (validee_par), INDEX region_idx (region), INDEX departement_idx (departement), INDEX categorie_idx (categorie), INDEX annonceur_idx (annonceur), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE sf_guard_group (id INT AUTO_INCREMENT, name VARCHAR(255), description TEXT, created_at DATETIME, updated_at DATETIME, PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE sf_guard_permission (id INT AUTO_INCREMENT, name VARCHAR(255), description TEXT, created_at DATETIME, updated_at DATETIME, PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE sf_guard_remember_key (id INT AUTO_INCREMENT, ip_address VARCHAR(50), user_id INT, remember_key VARCHAR(32), created_at DATETIME, updated_at DATETIME, INDEX user_id_idx (user_id), PRIMARY KEY(id, ip_address)) ENGINE = INNODB;
CREATE TABLE sf_guard_user (id INT AUTO_INCREMENT, username VARCHAR(128) NOT NULL, algorithm VARCHAR(128) DEFAULT 'sha1' NOT NULL, is_active TINYINT DEFAULT '1', is_super_admin TINYINT DEFAULT '0', salt VARCHAR(128), password VARCHAR(128), last_login DATETIME, created_at DATETIME, updated_at DATETIME, PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE sf_guard_group_permission ADD FOREIGN KEY (permission_id) REFERENCES sf_guard_permission(id);
ALTER TABLE sf_guard_group_permission ADD FOREIGN KEY (group_id) REFERENCES sf_guard_group(id);
ALTER TABLE sf_guard_remember_key ADD FOREIGN KEY (user_id) REFERENCES sf_guard_user(id);
ALTER TABLE sf_guard_user_group ADD FOREIGN KEY (user_id) REFERENCES sf_guard_user(id);
ALTER TABLE sf_guard_user_group ADD FOREIGN KEY (group_id) REFERENCES sf_guard_group(id);
ALTER TABLE sf_guard_user_permission ADD FOREIGN KEY (user_id) REFERENCES sf_guard_user(id);
ALTER TABLE sf_guard_user_permission ADD FOREIGN KEY (permission_id) REFERENCES sf_guard_permission(id);
